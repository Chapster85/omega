                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOMEGA ought to be enough for anybody." - Bill Gates, 2017
    ui->zOMEGApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOMEGASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOMEGA->setEnabled(false);
        ui->pushButtonMintzOMEGA->setToolTip(tr("zOMEGA is currently disabled due to maintenance."));

        ui->pushButtonSpendzOMEGA->setEnabled(false);
        ui->pushButtonSpendzOMEGA->setToolTip(tr("zOMEGA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOMEGApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOMEGA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOMEGA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOMEGA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOMEGA in ") + 
...
void PrivacyDialog::on_pushButtonSpendzOMEGA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOMEGA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOMEGA
        sendzOMEGA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOMEGA
    sendzOMEGA();
}

void PrivacyDialog::on_pushButtonZOMEGAControl_clicked()
{
    ZOMEGAControlDialog* zOMEGAControl = new ZOMEGAControlDialog(this);
    zOMEGAControl->setModel(walletModel);
    zOMEGAControl->exec();
}

void PrivacyDialog::setZOMEGAControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOMEGASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOMEGA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Omega Network Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOMEGApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOMEGA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OMEGA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOMEGA</b>";
    ...
    // use mints from zOMEGA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOMEGAControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOMEGAControlDialog::GetSelectedMints();
    }
    ...
    ZOMEGAControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOMEGA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " OMEGA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOMEGA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOMEGA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOMEGA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOMEGA "));
    ui->labelzOMEGAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOMEGASyncStatus->setVisible(fShow);
}
