...
std::list<std::string> ZOMEGAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOMEGAControlDialog::listMints;

ZOMEGAControlDialog::ZOMEGAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOMEGAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOMEGAControlDialog::~ZOMEGAControlDialog()
{
    delete ui;
}

void ZOMEGAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOMEGAControlDialog::updateList()
{
...
void ZOMEGAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOMEGAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOMEGA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOMEGAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOMEGAControlDialog::GetSelectedMints()
{
...
void ZOMEGAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
