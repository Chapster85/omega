...
int nAnonymizeOmegaNetworkAmount = 1000;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "omega.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty omega.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override omega.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "omegad.pid"));
